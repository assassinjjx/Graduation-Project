...
db.selectbook = function (userid, type, lwords, hwords, status, order, callback) {
    pool.getConnection(function (err, connection) {
        ...
        connection.query("SELECT b.*, (SELECT s.status FROM shelf s WHERE s.userid = ? AND s.bookid = b.id) AS shelfstatus FROM books b " +
            ...
            "ORDER BY " + order + " DESC", [userid, type, lwords, hwords, status], function (err, rows) {
                if (err) {
                    console.log(err);
                    callback(err, null);
                    return;
 ...
 * @return var
 */
db.updateuser = function (id, password, nickname, gender, old, callback) {
    pool.getConnection(function (err, connection) {
        if (err) {
            console.log(err);
            callback(err);
        ...
        connection.query("UPDATE users SET password = ?, nickname = ?, gender = ?, old = ? WHERE id = ?",
            [password, nickname, gender, old, id], function (err) {
                if (err) {
                    console.log(err);
                    callback(err);
                    return;
...
db.getshelf = function (userid, status, callback) {
    pool.getConnection(function (err, connection) {
        ...
        connection.query("SELECT (SELECT s.id FROM shelf s WHERE s.userid = ? AND s.bookid = b.id), b.id AS bookid, b.title, b.author, b.type, b.status, " +
            "b.words, b.latest, (SELECT c.star FROM comments c WHERE c.userid = ? AND c.bookid = b.id), b.titlelink, b.latestlink FROM books b " +
            "WHERE b.id in (SELECT s.bookid FROM shelf s WHERE s.userid = ? AND s.status = ?) ORDER BY id DESC",
...
db.getucomment = function (userid, callback) {
    pool.getConnection(function (err, connection) {
        ...
        connection.query("SELECT c.*, (SELECT b.title FROM books b WHERE b.id = c.bookid), (IF((SELECT COUNT(*) FROM praise p WHERE p.commentid = c.id AND p.userid = c.userid) > 0, 1, 0)) AS praisestatus FROM comments c WHERE c.userid = ? ORDER BY time DESC, id DESC",
...
db.getbcomment = function (userid, bookid, callback) {
    pool.getConnection(function (err, connection) {
        ...
        connection.query("SELECT c.*, (SELECT u.nickname FROM users u WHERE u.id = c.userid), (IF((SELECT COUNT(*) FROM praise p WHERE p.commentid = c.id AND p.userid = ?) > 0, 1, 0)) AS praisestatus FROM comments c " +
            "WHERE c.bookid = ? AND c.quality = false ORDER BY time DESC, id DESC", [userid, bookid], function (err, rows) {
                if (err) {
                    console.log(err);
                    callback(err, null);
                    return;
...
db.getbqcomment = function (userid, bookid, callback) {
    pool.getConnection(function (err, connection) {
        ...
        connection.query("SELECT c.*, (SELECT u.nickname FROM users u WHERE u.id = c.userid), (IF((SELECT COUNT(*) FROM praise p WHERE p.commentid = c.id AND p.userid = ?) > 0, 1, 0)) AS praisestatus FROM comments c " +
            "WHERE c.bookid = ? AND c.quality = true ORDER BY time DESC, id DESC", [userid, bookid], function (err, rows) {
                if (err) {
                    console.log(err);
                    callback(err, null);
                    return;
...
db.getubcomment = function (userid, bookid, callback) {
    pool.getConnection(function (err, connection) {
        ...
        connection.query("SELECT c.*, (SELECT u.nickname FROM users u WHERE u.id = c.userid), (IF((SELECT COUNT(*) FROM praise p WHERE p.commentid = c.id AND p.userid = c.userid) > 0, 1, 0)) AS praisestatus FROM comments c WHERE c.userid = ? AND c.bookid = ? " +
            "ORDER BY time DESC, id DESC", [userid, bookid], function (err, rows) {
                if (err) {
                    console.log(err);
                    callback(err, null);
                    return;
...
db.selectcomment = function (userid, callback) {
    pool.getConnection(function (err, connection) {
        ...
        connection.query("SELECT c.*, (SELECT u.nickname FROM users u WHERE u.id = c.userid), (SELECT b.title FROM books b WHERE b.id = c.bookid), " +
            "(IF((SELECT COUNT(*) FROM praise p WHERE p.commentid = c.id AND p.userid = ?) > 0, 1, 0)) AS praisestatus FROM comments c ORDER BY time DESC, id DESC LIMIT 500", [userid], function (err, rows) {
                if (err) {
                    console.log(err);
                    callback(err, null);
                    return;
...
db.getvs = function (userid, bookid, callback) {
    pool.getConnection(function (err, connection) {
        ...
        connection.query("SELECT v.*, (SELECT u.nickname FROM users u WHERE u.id IN (SELECT c.userid FROM comments c WHERE c.id = v.fcommentid)) AS fname, " +
            ...
            "IF((SELECT COUNT(*) FROM encourage e WHERE e.vsid = v.id AND e.userid = ?) > 0, 0, 1) AS votestatus " +
...
db.updatebstar(1, -1, function(err) {
    if(err) {
        console.log("11111");
    }
